...
import { ChevronUp, ChevronDown, Edit2, Save, X, Plus, Trash2 } from 'lucide-react';
...
interface StateAssignmentTableProps {
  isOpen: boolean;
  onClose: () => void;
}

const StateAssignmentTable: React.FC<StateAssignmentTableProps> = ({ isOpen, onClose }) => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      // Load state assignments from state_ref table
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-blue-100 text-sm">
                Excel-type sortable table for managing sales territories
              </p>
            </div>
            <button
              onClick={onClose}
        ...
        <div className="px-6 py-4 bg-gray-50 border-t">
          <div className="flex items-center justify-between text-sm text-gray-600">
            ...
            <div className="flex items-center gap-4">
              <div>Total Prospects: <span className="font-medium">{summary.totalProspects.toLocaleString()}</span></div>
              <div>Assignment Rate: <span className="font-medium">{states.length > 0 ? Math.round((summary.assigned / states.length) * 100) : 0}%</span></div>
            </div>
          </div>
        </div>
      </div>
...
export default StateAssignmentTable;